...
def extract_data_from_xml(xml_file, limit=None, start=0):
    ...
    for row in root.findall('.//row'):
        ...
        if accepted_answer_id:
            ...
            question_title = clean_html(row.get('Title'))
            question_body = clean_html(row.get('Body'))

            accepted_answer_row = root.find(f'.//row[@Id="{accepted_answer_id}"]')

            if accepted_answer_row is not None:
                accepted_answer_body = clean_html(accepted_answer_row.get('Body'))
                data.append({
...
data_db = extract_data_from_multiple_files(input_xml_file1, input_xml_file2, limit=20000, start=5000)
data_train = extract_data_from_multiple_files(input_xml_file1, input_xml_file2, limit=20000, start=25000)
...
output_db_excel = 'stackExchangeQsAndAnswersDB.xlsx'
output_db_json = 'stackExchangeQsAndAnswersDB.json'
save_to_excel(data_db, output_db_excel)
save_to_json(data_db, output_db_json)
...
print(f"Extracted and saved 5,000 questions and answers to {output_test_excel} and {output_test_json}.")
print(f"Extracted and saved 20,000 questions and answers to {output_db_excel} and {output_db_json}.")
